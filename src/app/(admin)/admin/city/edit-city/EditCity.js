...
const EditCity = () => {
  ...
  const getCityData = async () => {
      try {
        const response = await axios.post("/api/admin/city/cityById", {
          id: id,
        });
        if (response.data.status === 1) {
          setState(response.data.data.cities.state);
          setDistrict(response.data.data.cities.district); 
          setDistrictList(response.data.data.districts); 
          setCity(response.data.data.cities.city);        
        }
      } catch (error) {
        console.error(
          "Error fetching city:",
